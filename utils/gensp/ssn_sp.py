...
def get_abs_indices(init_label_map, num_spixels_width):
    ...
    abs_pix_indices = torch.arange(n_pixel, device=device)[None, None].repeat(b, 9, 1).reshape(
        -1).long()  # b,9,H*W，像素序号
    ...
    abs_spix_indices = (init_label_map[:, None].float() + relative_spix_indices[None, :, None].float()).reshape(
        -1).long()
...
def ssn_iter(pixel_features, num_spixels=200, n_iter=10):  # num_spixels=128
    """
    ...
    Args:
        pixel_features: torch.Tensor
            A Tensor of shape (B, C, H, W) (4, 256, 137, 100)
        num_spixels: int
            A number of superpixels 128
        n_iter: int

...
#     input_pd = F.pad(input.unsqueeze(0).unsqueeze(0).float(), (w_shift_unit, w_shift_unit, h_shift_unit, h_shift_unit),
#                      'replicate')
...
#
#     pixel_surrounding_idxs_repeat = pixel_surrounding_idxs.unsqueeze(-1).repeat(1, 1, 1, c).reshape(b, -1, c).type(
#         torch.int64)  # b,n,9->b,n,9,c->b,n*9,c
#
#     p_features = p_features.reshape(b, c, n).permute(0, 2, 1)  # b, n, c
#
#     for i in range(n_iter):
#         surrounding_c_features = torch.gather(c_features, dim=1, index=pixel_surrounding_idxs_repeat).reshape(b, n, -1,
#                                                                                                               c).permute(
#             0, 1, 3, 2)  # b, n*9, c -> b, n, 9, c -> b, n, c, 9
#
...
#
#     pixel_surrounding_idxs = pixel_surrounding_idxs.reshape(b, -1, h * w).permute(0, 2, 1).type(
#         torch.int64)  # b, 9, h, w -> b, 9, n -> b, n, 9
#
#     # 迭代聚类，生成相似度矩阵
#
...
class DWConv(nn.Module):
    def __init__(self, dim=1024):
        ...
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)  # 分组卷积
...
class MHCA(nn.Module):  # 多头交叉注意力机制
...
class GlobalAggregation(nn.Module):
    ...
    def forward(self, des, x):
        # print("des.shape", des.shape) 4,117,256
        # print("x.shape", x.shape) 4,256,137,100
        c, h, w = x[0].shape
        ...
        for i in range(batch):
            ...
            decoder_q = xi.unsqueeze(0)  # 1,n,c (1,13700,256)
...
class RegionAdaption(nn.Module):
    ...
    def forward(self, sp, x, xp):  # sp(137,100) x(256,137,100) xp(256,137,100)
        c, h, w = x.shape

        x = x.permute(1, 2, 0).reshape(-1, c)  # hw,c
        xp = xp.permute(1, 2, 0).reshape(-1, c)

        sp_values = torch.unique(sp)

        sp_num = sp_values.shape[0]  # 117
        # print("sp_num",sp_num)
        ...
        for i in range(sp_num):
            ...
            # print("region_pos_2d.shape", region_pos_2d.shape) [nums,2]
            region_pos_1d = region_pos_2d[:, 0] * w + region_pos_2d[:, 1]

            encoder_qkv = x[region_pos_1d] + xp[region_pos_1d]  # 区域内特征+区域的position_embedding
        ...
        # print("d_r.shape", d_r.shape) [117,256]
...
class posi_attn(nn.Module):
    ...
    def __init__(self, args, channel):
        ...
        self.pos = DWConv(channel)  # position_embedding

        self.ra = RegionAdaption(args, channel)
        self.ga = GlobalAggregation(args, channel)

    def forward(self, x):
        b, _, x_h, x_w = x.shape  # 4,256,137,100
        # np.save('F_feature.npy',x.squeeze(0).detach().cpu().numpy())

        xp1 = self.pos(x)
        # print('xp1.shape', xp1.shape) 4,256,137,100
        abs_affinity, sps, spixel_features = ssn_iter(x, num_spixels=self.args.groups, n_iter=10)  # group 区域的数量

        # print("abs_affinity.shape", abs_affinity.shape)
        # print("sps.shape", sps.shape)  # 4,13700
        # print("max_sps", torch.max((sps)))
        ...
        # print(torch.max(sps)) 117

        sps = sps.reshape(b, x_h, x_w)  # 4,137,100
...
class rat_model(nn.Module):
    ...
    def __init__(self, args, classes, in_channels, freeze_bn=False, freeze_backbone=False):  # in_channels=3

        super(rat_model, self).__init__()

        self.args = args

        self.backbone_name = args.backbone  # vgg16
        ...
        self.pa = posi_attn(args, self.backbone.channels[-1] // 2)  # 里面包含了RAC和GAC
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.shape
        x = self.backbone(x)  # return [x0, x1, x2, x3],所以x[-2]就是x2
        if self.training:
            x_dsn = self.conv_dsn(x[-2])  # x[-2].shape torch.Size([4, 256, 137, 100])
            x_dsn = F.interpolate(x_dsn, size=(h, w), mode='bilinear', align_corners=False)  # 双线性插值 这里对应模型vgg16倒数输出的值

        x = x[-1]  # x[-1].shape: torch.Size([4, 512, 137, 100])
        _, _, x_h, x_w = x.shape
        ...
        x = self.conv_out(torch.cat([x_pa, x], 1))  # 沿着通道拼接
        x = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=False)  # 双线性插值还原到原来尺寸
        if self.training:
            return x, x_dsn
        else:
            return x
    ...
    def _initialize_weights(self):
        modules = [self.conv, self.pa, self.conv_dsn, self.conv_out]  # , self.ca
